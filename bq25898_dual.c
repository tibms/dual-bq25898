...
static int bq2589x_adjust_hvdcp_output(struct bq2589x *bq, u8 dp, u8 dm)
{
	u8 val;
	int ret;

	val = (dp << BQ2589X_DPDAC_SHIFT ) | (dm << BQ2589X_DMDAC_SHIFT);

	ret = bq2589x_update_bits(bq, BQ2589X_REG_01, BQ2589X_DPDAC_MASK | BQ2589X_DMDAC_MASK, val);
	return ret;
}
EXPORT_SYMBOL_GPL(bq2589x_adjust_hvdcp_output);
...
static int bq2589x_enable_12v_handshake(struct bq2589x* bq, bool enable)
{
	...
	ret = bq2589x_update_bits(bq, BQ2589X_REG_01, BQ2589X_EN12V_MASK, val);
...
static int bq2589x_parse_dt(struct device *dev, struct bq2589x *bq)
{
	...
	bq->cfg.enable_12v = of_property_read_bool(np, "ti,bq2589x,enable-12v-handshake");
...
static void bq2589x_adapter_in_workfunc(struct work_struct *work)
{
	...
	if (bq->vbus_type == BQ2589X_VBUS_MAXC) {
		...
		/*if you want adjust hvdcp adapter output voltage manually, do it here*/
/*		bq2589x_adjust_hvdcp_output(bq,BQ2589X_DP_27000MV, BQ2589X_DM_0MV); */
...
static void bq2589x_monitor_workfunc(struct work_struct *work)
{
	...
	if (g_bq2->enabled && g_bq1->rsoc > 95) {
		...
		if (bq->vbus_type == BQ2589X_VBUS_MAXC) {
			bq2589x_adjust_hvdcp_output(bq,BQ2589X_DP_0MV, BQ2589X_DM_0MV); /* reset to 5V */
		} else if (bq->vbus_type == BQ2589X_VBUS_USB_DCP && pe.enable && !pe.tune_down_volt) {
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (!ret && bq->part_no == BQ25898) {
		...
		dev_info(bq->dev, "%s: no charger device bq25898 found:%d\n", __func__, ret);
		return -ENODEV;
	}
